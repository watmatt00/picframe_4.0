...
- [y ] `GET http://192.168.102.210:8000/health` returns `{"status": "ok"}` with HTTP 200
- [ y] `GET http://192.168.102.210:8000/version` returns `{"version": "4.0.0", "api": "picframe"}` with HTTP 200
- [y ] Dashboard loads at `http://192.168.102.210:8000/` without authentication
...
- [x ] Navigate to `http://192.168.102.210:8000/pairing`
- [x ] QR code image is displayed and non-empty
- [x ] Manual code is displayed in `ABC-XYZ` format (6 alphanumeric characters)
- [x ] Expiry countdown is shown (starts at ~5 minutes)
- [x ] Funnel URL is displayed on the page
- [x ] 4-step pairing instructions are visible
- [x ] "Regenerate Code" button generates a new code
- [ ] Page auto-reloads when countdown expires <!-- FAIL: require button push to regenerate code -->

#### iOS: Pair a Frame

- [x ] Open app → Frame list shows empty state with "Pair a Frame" button
- [x ] Tap "Pair a Frame" → PairingView opens
- [ ] Enter Tailscale IP, pairing code, and device name <!-- FAIL: replace ip field w/ url -->
- [x ] "Pair Device" button is disabled until all 3 fields are filled
- [x ] Pairing code field validates 6-7 character length
- [x ] Tap "Pair Device" → spinner appears during pairing
- [x ] On success: frame appears in frame list with name, role, IP
- [x ] JWT token is stored in iOS Keychain (verify via frame operations working)
- [x ] After pairing, app auto-fetches cloud credentials and source list

#### Negative Cases

- [x ] Wrong pairing code → error message displayed, frame not added
- [x ] Expired code (wait >5 min) → error message displayed
- [ ] Rate limit: generate 3 codes within an hour, 4th attempt returns 429 <!-- FAIL: error should be more descriptive -->
- [x ] **Negative:** Empty fields → "Pair Device" button stays disabled
...
- [x ] Traffic light indicator is visible (circle/dot)
- [x ] Traffic light is GREEN when sync status = "match" and counts match
- [x ] Traffic light is AMBER when local and remote counts differ
- [x ] Traffic light is RED when sync error has occurred
- [ ] Traffic light is BLUE (animated) when sync is in progress <!-- FAIL: manual sync did not change color.  refresh interval?? -->
- [x ] Cloud photo count is displayed and matches remote count
- [x ] Frame (local) photo count is displayed and matches local count
- [x ] Current display source name is shown
- [x ] Current image thumbnail is displayed (from Pi3D)
- [ ] showlogs toggles log detail view <!-- FAIL: this entry needs to be added to test template -->
#### Service Health

- [ x] PicFrame Display service status shown (running/stopped)
- [ x] PicFrame API service status shown (running/stopped)

#### Storage

- [ x] Storage usage shown in GB (e.g., "12.5 GB / 64.0 GB")
- [ x] Storage percentage displayed (e.g., "19.5%")

#### Quick Actions

- [ ] "Sync Now" button triggers sync → traffic light goes BLUE <!-- FAIL: manual sync did not change color.  refresh interval?? -->
- [ ] After sync completes, traffic light returns to appropriate color <!-- FAIL: manual sync did not change color.  refresh interval?? -->
- [ x] "Restart Frame" button restarts picframe service
- [ x] "Restart API" button restarts picframe-api service (page may briefly disconnect)
- [ x] "Refresh" button updates all displayed data

#### Source Switching

- [ x] Source dropdown/selector lists all configured sources
- [ x] Switching source updates "Current Source" display
- [ x
] Switching source auto-triggers sync for the new source

#### Advanced Mode

- [ x] Advanced toggle reveals technical details (JSON, API paths)
...
- [ ] Frame ID displayed (read-only) <!-- FAIL: not shown.  part of legacy menu -->
- [x] Frame Name displayed and editable
- [ ] Funnel URL displayed (read-only) <!-- FAIL: not shown -->
- [ ] Rotation Interval field shown with current value (seconds) <!-- FAIL:  update header to "Picture Rotation" -->
- [x]Rotation Interval accepts values 5-3600
- [ ] Sync Interval field shown with current value (seconds) <!-- FAIL:  update header to clouud syn-->
- [x] Sync Interval accepts values 60-86400
- [x] Log Level dropdown: DEBUG, INFO, WARNING, ERROR
- [] Submit form → success message displayed  <!-- FAIL: legacy vs refactored??? -->
- [x] Reload page → saved values persist
- [x] **Negative:** Rotation interval < 5 → validation error
- [x] **Negative:** Sync interval < 60 → validation error
...
- [x] Page lists all paired devices
- [x] Each device shows: name, role, paired date, last seen
- [x] "Revoke" button present for each device
- [x] Revoke a device → device removed from list
- [x] After revoking, that device's JWT no longer works (app shows unauthorized)
- [] **Negative:** Cannot revoke the last admin device → error message <!-- FAIL: this error needs to be more apparent-->
...
- [x ] Page loads with operations log displayed by default
- [ x] Toggle between "Operations" and "Security" log types
- [ x] Line count selector works (adjustable 1-500)
- [ x] Log entries display with timestamps
- [ x] Security log shows pairing attempts and auth events
- [ x] Operations log shows sync events and service actions
- [ x] Refresh loads latest entries
